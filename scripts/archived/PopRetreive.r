...
#' the data frames available from the [population] list.
#'
...
#'
#'
#'  # Get the census population for Dalls and Denton counties in 2000, 2010 and 2020
...
retrieve_county_population <- function(.pop_df,
  ...
  assert_that(.pop_df %has_name% names(tarr::population$census),
  ...
  arg_check <- map_lgl(names(argument_list) ,\(x) any(argument_list[[x]] %in% avail[[x]]))
  if(! all(arg_check)) {
  ...
  # Age or age group processing
  if(! is.null(.age.groups)){
    argument_list$age.groups = .age.groups

    ret <- ret |>
      filter(age.iv != iv(0,Inf)) |>
      mutate(age.iv = age_cat_iv(age = age.iv, age_group = .age.groups)) |>
      reframe(population = sum(population), .by = c(year, fips, area.name, sex, race, ethnicity, age.iv))  |>
      filter(! is.na(age.iv)) |>
      mutate(age.char = age_group_to_char(age.iv) |>
               ordered() |>
               forcats::fct_reorder(age.iv, .fun = \(v) iv_start(v) |> median())) |>
      identity()
  ...
  # return the filtered data frame with fileds inte same order as .pop_df
...
#' Used internally
#'
#' @param df is the data frame in from the population list
...
get_available_values <- function(df) {
  ...
  levels_or_unique <- function(field){
    ...
    #browser()
    ret <- switch(EXPR = cl,
      ordered   = levels(field),
      factor    = levels(field),
      integer   = unique(field) |> sort(),
...
#' @param .age.groups is vector of integers defining the age group.  See the [age.groups] list for pre-defined age
#'   groups.
...
#'   External use plots,
#'   \itemize{
#'        \item Incidence for less than .suppress.level cases results in suppression.  Caution should be used with this
#'        chart as suppressed bars could be mis-interpreted as zero cases.  The total number of cases for each year are
#'        shown unless the total is below .suppress.level
#'        \item Rates are calculated with a minimum of .suppress.level cases and shown with a hollow bar for display
#'        purposes.
#'        }
#'
#'    Internal use plots
#'      \itemize{
#'        \item Incidence, all case counts are shown regardless of .suppress.level \item Rates are shown, however those
#'        calculated with .suppress.level or fewer cases are labeled as unstable and the bar displayed with increased
#'        transparency.  The legend will show these bars as "Unstable"
#'        }
#'
#' @seealso \code{\link{age.groups}}, variable to use pre-defined age cateogories
#' @export
plot_age_group_year <- function(.df, .dateFld, .ageFld, .condition,
                                .fill = "navy",
                                .years = 2010:lubridate::year(lubridate::today()),
                                .age.groups = age.groups$Yr.10, .rates = TRUE,
                                .external = TRUE,
                                .suppress.level = 10,
  ...
  assert_that(is.character(.condition))
  assert_that(is.numeric(.years) & length(.years) >0)
  assert_that(base::tolower(.fill) %in% grDevices::colors())
  assert_that(is.numeric(.age.groups))
  assert_that(is.logical(.external))
  ...
  if(.rates){
    ...
    age.grp.pop <- tarr::retrieve_county_population(.year = .years,
                                                    .ageGrp = .age.groups,
                                                    .county = .county) %>%
      ungroup() %>%
...
retrieve_zip_code_population <- function(.endYear, .area = "Tarrant"){
  ...
  assert_that( is.scalar(.emdYear))

  if(! all(.endYear %in% unique(population$tarrant.zcta$end.year))){
      rng <- range(population$tarrant.zcta$end.year)
      stop("The .endYear argument was not in the range of ", rng[1]," - ",rng[2])
  }

  assert_that(.area %in% c("Tarrant"))
