...
get_census_age_race <- function(dec_year = 2010, 
  ...
  if(! all(st %in% fips_codes$state %>% unique())){
    ...
    stop(paste("state(s)", paste0("'", mssg, "'", collapse = ","), "is not valid"))
  }
  ...
  if(! all(county_full %in% fips$county) & geography == "county"){
    ...
    stop(paste("County(s)", paste0("'",mssg, "'", collapse = ","), "are not valid"))
  }
  ...
  format_label    <- compose(~str_remove(., ":$"),
                             ~str_remove(., "total_"),
                             ~str_replace_all(., c("^_{1,2}" = "", 
                                                   ":_" = "_", 
                                                   ";"  = "",
                                                   ":$" = "")),
                             str_to_lower,
  ...
  vars <- load_variables(year = dec_year, dataset = dataset, cache = T )
  ...
  age_sex_vars <- vars %>% 
   ...
   pop <- get_decennial(year = dec_year, sumfile = dataset,
                       state = st_fips, 
                       county = county_fips, 
    ...
    left_join(., vars %>% select(name, label, concept), 
              by  = c("variable" = "name")) %>% 
    mutate(label = paste(format_label(label), 
           extract_concept(concept), sep = "_") %>% 
             str_remove(., "_$")  
     ) %>% 
    select(-concept)
...
census <- bind_rows(counties_pop, texas_pop) |> 
  ...
  mutate(area.name = str_remove(area.name, " County, Texas"),
         ...
         fips = geoid,
         age.iv   = tarr.population::as.age_group(age.char),
         age.char  = tarr.population::iv_to_age_group(age.iv)
  ) |> 
  select( year, fips, area.name, sex, age.char, age.iv, race.eth, population)
...
total_sex <- census |>
  ungroup() |> 
  filter(age.char != "All") |>
  summarise(population = sum(population), .by = c(year, fips, area.name, age.char, age.iv, race.eth)) |> 
  mutate(sex = "All") |> 
  select(names(census))

# add total_sex to census
census <- bind_rows(census, total_sex) |> 
  arrange(area.name, race.eth, sex, age.iv)
...
census_list <- census |> 
  filter(race.eth %nin%  c("All", "Hispanic")) |> 
...
census <- set_source_url(
  census, 
  nm = "Decennial Censuses ", 
  url = "Used the tidyensus package for retrieval")


rm(age_string, rac_eth_string, extract_ethnicity,
   get_census_age_race, counties_pop, fips_codes, years)
rm(list = ls(pattern = "extract_+"), census_list, texas_pop, cnts_to_get)
...
# A new column is added to account for age groups as Intervals, supported by the ivs package
...
# The first file is the latest estimates for the 2010s update.
# The second file has the latest estimates for the 2020s
est_fns <- list(
  est_2010_2019 = "cc-est2020-alldata-48.csv",
  est_2020s     = "cc-est2023-alldata-48.csv"
)
...
census.estimates <- bind_rows(est_list) |> 
  ...
  select(fips = county, area.name, year, sex, age.char = agegroup, age.iv, 
         race, ethnicity, population = estimate) |> 
  # place fields in same order as census data frame
  select(all_of(names(census)))
  
census.estimates <- set_source_url(
  obj = census.estimates, 
  nm = "Census Bureau estimates",
  url = "https://www.census.gov/programs-surveys/popest/data/tables.html")


rm(read_estimate_file, entries_in_2020s, years_name_vec, est_list,
...
tarrant.zip.codes.estimates <- set_source_url(
  obj = tarrant.zip.codes.estimates, 
  url = "https://data.census.gov/all?q=Population+by+age+in+ZCTA",
  nm = "ACS ZCTA population 5 year period estimates 2011 - 2021")
attributes(tarrant.zip.codes.estimates)
rm(tarrant.zip.codes,get_tarr, years.zip.codes, counties, sort_values)

#map(list(pop, census, census.estimates), names)

print("END of population construction script")
